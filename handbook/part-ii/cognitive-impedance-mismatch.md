...
So, narrowly: Cognitive Impedance Mismatch is the **dynamic friction** that arises when a biological mind and a computational instance — built to different scales of processing volume and speed — fail to mesh across the interface between them. It is a property of the interface, not of either side on its own; and it is not a static gauge of that gap but the live grinding the gap produces when the two try to think together. The difference in scale is the *origin*; the mismatch is the friction it generates in motion.
...
It is not the same thing as [Cognitive Rhythm](/en/handbook/part-ii/cognitive-rhythm), though the two are close enough to blur. Rhythm is the temporal *pattern* of the human–machine coupling — the alternation of processing, friction, and expression cycles, the rhythmic structure that *can* mesh and run well. Rhythm is the music. Impedance mismatch is the dissonance: the friction that arises, human↔machine, when the biological and machine scales do *not* mesh — and that friction includes the temporal grinding, not only the bare difference in **how much** each side can hold and **how fast** it moves it. The scale difference is where the dissonance comes from; the mismatch is the dissonance itself, the grinding you hear when the two clocks fail to interlock. So the rhythm page is right to call the grinding *this* mismatch — the grinding is part of what the term names.

And it is not [Cognitive Friction](/en/handbook/part-ii/cognitive-friction). Friction, in the handbook's sense, is the *productive* resistance a learner meets and works through — something the handbook wants to preserve. The friction of an impedance mismatch is a different animal: it is the unproductive grinding two mismatched scales produce, the friction that has to be engineered down rather than worked through. You can convert that grinding into productive friction (that is, in part, what the protocols are for), but the scale difference it springs from is the standing precondition — no working-through dissolves the difference in channel size itself.
...
If the mismatch were only a scale difference sitting still, it would not need a page. It needs one because the friction it produces has a default failure mode, and naming it is the first defense against that mode.

The default is to let the larger channel set the terms. The model can produce five hundred pages, so it produces; the human, unable to hold or contest that volume, accepts it — not because it is right but because contesting it would mean re-deriving it, and the channel that would do the re-deriving is the narrow one. This is how the mismatch quietly slides into the **Deference Trap**, the human face of [the Compliance Trap](/en/handbook/part-vi/compliance-trap) — the human surrendering to the machine, deferring rather than doing friction: the human stops processing and starts ratifying, because ratifying is the only operation the narrow channel can afford against the wide one. The fix is not a bigger human or a smaller model. It is to treat the interface itself as the thing to engineer — to deliberately throttle, to chunk the flood back down to channel width, to make the model return less so the human can hold and resist what it returns. Several of the handbook's patterns are exactly that engineering: the [Context-Window Economy](/en/handbook/part-iii/context-window-economy) rations what crosses the interface, and the [Blues Protocol](/en/handbook/part-ii/blues-protocol) is in part a discipline for matching tempos across a gap the two sides cannot close on their own.

So the term does one job. It locates the friction in the coupling between the participants — at the wire, where the two scales grind — which is the only place it can actually be worked.
